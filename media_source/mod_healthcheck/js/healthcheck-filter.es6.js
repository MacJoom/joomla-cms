  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    function initFilterBar(filterBar) {
      ...
      filterBar.addEventListener('click', (event) => {
        ...
        if (button.hasAttribute('data-healthcheck-refresh')) {
          event.preventDefault();
          ...
          /*
           * Re-run every asynchronous check. Results which were rendered server-side cannot be
           * refreshed in place, so when the module carries no asynchronous icon at all the page is
           * reloaded instead - otherwise the button would claim a refresh it never performed.
           */
          if (document.querySelector('.health-checks [data-url]')) {
            document.dispatchEvent(new CustomEvent('joomla:healthcheck-refresh'));
          } else {
